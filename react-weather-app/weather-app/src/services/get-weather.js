...
export const getWeather = async (place) => {
    // console.log("called from: ", place);
    const { name, lat, long } = place;
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${long}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,rain,weather_code,wind_speed_10m`

    const result = await fetch(url);
    // console.log(await result.json());
    const data = await result.json();
    const curr = data.current;
    console.log(curr.weather_code)
    if (!curr) {
        throw new Error("Weather details not received!")
    }

    const weather = WMO_CODES[curr.weather_code]
    const icon = weather.icon === "clear" && curr.is_day === 0 ? "clear_night" : weather.icon;
